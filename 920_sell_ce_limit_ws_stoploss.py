...
STRATEGY = os.getenv("STRATEGY_NAME", "920 Sell CE Limit WS SL")
...
ENTRY_TIME = os.getenv("ENTRY_TIME", "09:20")
EXIT_TIME = os.getenv("EXIT_TIME", "15:15")
STOPLOSS_PERCENT = float(os.getenv("STOPLOSS_PERCENT", "50"))

# Aggressive limit offsets. For SELL, price = LTP - offset. For BUY exits,
# price = LTP + offset. Orders remain LIMIT orders.
ENTRY_LIMIT_OFFSET_POINTS = float(os.getenv("ENTRY_LIMIT_OFFSET_POINTS", "1"))
EXIT_LIMIT_OFFSET_POINTS = float(os.getenv("EXIT_LIMIT_OFFSET_POINTS", "1"))
TICK_SIZE = float(os.getenv("TICK_SIZE", "0.05"))

ENTRY_FILL_TIMEOUT_SECONDS = float(os.getenv("ENTRY_FILL_TIMEOUT_SECONDS", "20"))
EXIT_FILL_TIMEOUT_SECONDS = float(os.getenv("EXIT_FILL_TIMEOUT_SECONDS", "10"))
EXIT_REPRICE_ATTEMPTS = int(os.getenv("EXIT_REPRICE_ATTEMPTS", "10"))
...
def on_ltp(message):
    ...
    if should_exit:
        threading.Thread(
            ...
            args=(f"stop-loss hit: LTP {ltp:.2f} >= {stoploss_price:.2f}",),
            daemon=True,
        ).start()
...
def exit_with_limit_orders(reason):
    ...
    for attempt in range(1, EXIT_REPRICE_ATTEMPTS + 1):
        ...
        exit_limit = max(TICK_SIZE, ltp + EXIT_LIMIT_OFFSET_POINTS)
